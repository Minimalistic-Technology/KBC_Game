...
export default function GameConfigPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      // --- Only show question banks that have been published ---
  ...
  // --- Keep prizeLadder length in sync with selectedBankIds length ---
  useEffect(() => {
    if (!currentFullConfig) return;

    const selectedCount = currentFullConfig.selectedBankIds?.length ?? 0;
    const existing = currentFullConfig.prizeLadder || [];

    // Already in sync → do nothing
    if (selectedCount === existing.length) return;

    const synced: PrizeLevel[] = [];

    // 1) Keep as many existing levels as needed (slice if too many)
    const keepCount = Math.min(selectedCount, existing.length);

    for (let i = 0; i < keepCount; i++) {
      const lvl = existing[i];
      synced.push({
        ...lvl,
        // level must always be 1..N in order
        level: i + 1,
      });
    }

    // 2) If we need more levels, add new default ones
    if (selectedCount > keepCount) {
      // Get max existing id so new ids don't clash
      let nextId = existing.reduce((max, lvl) => (lvl.id > max ? lvl.id : max), 0) || 0;

      for (let i = keepCount; i < selectedCount; i++) {
        nextId++;

        synced.push({
          id: nextId, // local React id
          level: i + 1, // 1-based level
          type: 'money', // default
          value: 0, // default amount
          giftDesc: '', // empty by default
          isSafe: false,
          // mongoId: undefined,
          // media: undefined,
        });
    ...
    // 3) Update config with the exact synced ladder
    updateCurrentConfig('prizeLadder', synced);
  }, [currentFullConfig?.selectedBankIds?.length]);
  ...
  const filteredBanks = useMemo(() => {
    ...
    return banks.filter((bank) => {
      const searchMatch =
        searchTerm === '' || bank.title.toLowerCase().startsWith(searchTerm.toLowerCase());
  ...
  const handleLifelineToggle = (label: keyof Lifeline) => {
    if (!currentFullConfig) return;
    const newLifelines = {
      ...currentFullConfig.lifelines,
      [label]: !currentFullConfig.lifelines[label],
    };
    updateCurrentConfig('lifelines', newLifelines);
  };

  // Clamp prize ladder so it never exceeds selectedBankIds length
  const handlePrizeLadderChange = (newLadder: PrizeLevel[]) => {
    if (!currentFullConfig) return;

    const targetLen = currentFullConfig.selectedBankIds?.length ?? 0;
    const existing = currentFullConfig.prizeLadder || [];

    // If no banks selected → ladder must be empty
    if (targetLen === 0) {
      updateCurrentConfig('prizeLadder', []);
      return;
    }

    const synced: PrizeLevel[] = [];

    // 1) Use only first N levels coming from editor
    const keepCount = Math.min(targetLen, newLadder.length);

    for (let i = 0; i < keepCount; i++) {
      const lvl = newLadder[i];
      synced.push({
        ...lvl,
        level: i + 1, // normalize level numbers
      });
    }

    // 2) If editor has fewer than targetLen, fill with defaults
    if (targetLen > keepCount) {
      // Compute max id from all known levels so ids don't clash
      let nextId = [...existing, ...newLadder].reduce(
        (max, lvl) => (lvl.id > max ? lvl.id : max),
        0,
      );

      for (let i = keepCount; i < targetLen; i++) {
        nextId++;
  ...
  const createConfigMutation = useMutation({
    mutationFn: async (newConfigData: Omit<GameConfig, 'id'>) => {
      const { data } = await api.post<{ config: BackendConfig }>(
        '/api/v1/game-config',
        frontendToBackend(newConfigData as GameConfig),
      );
      return data.config;
    },
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="p-2 rounded-md hover:bg-slate-100 text-slate-500 hover:text-indigo-600 disabled:opacity-50"
              >
                {createConfigMutation.isPending ? (
            ...
            {isEditorLoading ? (
              ...
              <>
                <div>
                  ...
                  <input
                    type="text"
                    id="configName"
                    value={currentFullConfig.name || ''}
                    onChange={(e) => updateCurrentConfig('name', e.target.value)}
                    className="w-full text-2xl font-bold border-0 border-b-2 p-1 focus:ring-0 focus:border-indigo-500 text-slate-900"
                  />
                </div>
                <div className="pt-6 border-t">
                  <h3 className="text-lg font-bold text-slate-900 mb-4">
                    Select Question Banks (in order)
                  </h3>
                  {/* Bank filters */}
                    ...
                    <div className="col-span-1 md:col-span-3">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full h-10 pl-10 pr-4 border bg-white border-slate-300 rounded-lg text-slate-900"
                        />
                      </div>
                    </div>
                    <div>
                  ...
                  {isBanksLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {filteredBanks.map((bank) => {
                        const isSelected =
                          currentFullConfig.selectedBankIds.includes(bank.id);
                        const order = isSelected
                          ? currentFullConfig.selectedBankIds.indexOf(bank.id) + 1
                          : 0;
                        return (
                          <BankSelectionCard
                            key={bank.id}
                ...
                <div className="pt-8 border-t">
                  <PrizeLadderEditor
                    value={currentFullConfig.prizeLadder}
                    onChange={handlePrizeLadderChange}
                    configId={currentFullConfig.id}
                  />
                </div>
                <div className="pt-8 border-t">
                  <h3 className="text-lg font-bold text-slate-900 mb-4">
                    Configure Available Lifelines
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <LifelineToggle
                      label="50:50"
                      icon={Zap}
                      isEnabled={currentFullConfig.lifelines['50:50']}
                    ...
                    <LifelineToggle
                      label="Expert Advice"
                      icon={Lightbulb}
                      isEnabled={currentFullConfig.lifelines['Expert Advice']}
                      onToggle={handleLifelineToggle}
                    />
                    <LifelineToggle
                      label="Flip Question"
                      icon={RefreshCw}
                      isEnabled={currentFullConfig.lifelines['Flip Question']}
                      onToggle={handleLifelineToggle}
                    />
                  </div>
                </div>
              ...
              <div className="flex flex-col justify-center items-center text-center py-20">
                ...
                <p className="mt-1 text-slate-500">
                  Select a configuration from the list or create a new one to begin.
                </p>
              </div>
            )}
          </div>
