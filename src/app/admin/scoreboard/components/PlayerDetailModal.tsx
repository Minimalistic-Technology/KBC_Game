...
import { Score } from '@/app/admin/scoreboard/components/data';
import { X, Clock, Zap, Target, HelpCircle } from 'lucide-react';
        ...
        <div className="flex items-start justify-between p-5 border-b rounded-t">
          <div>
            ...
            <p className="text-sm text-slate-600">
              {score.questionBank} - {new Date(score.date).toLocaleString()}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div className="p-4 bg-slate-50 rounded-lg">
              <Target className="mx-auto h-6 w-6 text-indigo-600" />
            ...
            <div className="p-4 bg-slate-50 rounded-lg">
              ...
              <p className="text-2xl font-bold text-slate-800 mt-2">{score.totalTimeSeconds}s</p>
              <p className="text-xs text-slate-500">Total Time</p>
            </div>
             <div className="p-4 bg-slate-50 rounded-lg">
              <HelpCircle className="mx-auto h-6 w-6 text-indigo-600" />
              <p className="text-2xl font-bold text-slate-800 mt-2">{score.questions.length}</p>
          ...
          <div>
            ...
            <div className="space-y-3">
              {score.questions.map((q: Score['questions'][number], index: number) => (
                <div key={index} className="flex items-center gap-4 p-3 bg-white border rounded-md">
                   ...
                   <div className="text-right">
                      <p className="font-semibold text-sm text-slate-700">{q.timeTakenSeconds}s</p>
