...
export const QuestionBankCard = ({ bank, index, onEdit, onDelete, onMove, isFirst, isLast }: CardProps) => {
  // CORRECTED: Keys are now lowercase for case-insensitive matching
  ...
  // Create a safe, lowercase key for the lookup
    ...
    >
        <Link href={`/admin/questions?bankId=${bank.id}`} className="flex-grow">
            <div className="p-6">
                <div className="flex justify-between items-start mb-3">
                    ...
                    {/* CORRECTED: Using the safe, lowercase key to apply styles */}
        ...
        <div className="border-t border-slate-200 bg-slate-50/75 p-3 flex justify-between items-center text-sm">
            <div className="flex items-center gap-4 text-slate-700 font-medium">
                ...
                <span className="flex items-center gap-1.5" title="Prize money">${(bank.prize || 0).toLocaleString()}</span>
            </div>
