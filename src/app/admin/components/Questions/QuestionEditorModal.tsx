...
export const QuestionEditorModal = ({ question, onSave, onClose }: { question: Question, onSave: (question: Question) => void, onClose: () => void }) => {
  ...
  const handleMediaUploadComplete = (asset: MediaAsset) => { setFormData(prev => ({ ...prev, media: asset })); };
  const handleSelectFromLibrary = (asset: MediaAsset) => { setFormData(prev => ({ ...prev, media: asset })); setIsLibraryOpen(false); };
  const handleRemoveMedia = () => { setFormData(prev => ({ ...prev, media: undefined })); };
  const handleDefaultFormatChange = (format: DerivedFormat) => { if (formData.media) { setFormData(prev => ({ ...prev, media: { ...prev.media!, defaultFormat: format } })); } };
  ...
  const handleAnswerChange = (option: string) => { setFormData(prev => ({ ...prev, answer: option })); };
  ...
  return (
    <>
      <AnimatePresence>
        ...
        {isLibraryOpen && ( <MediaLibraryModal onClose={() => setIsLibraryOpen(false)} onSelect={handleSelectFromLibrary} filter="all" /> )}
      </AnimatePresence>
